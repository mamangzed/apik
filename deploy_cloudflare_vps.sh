...
INTERCEPT_PROXY_HTTPS_MITM_ENABLE="${INTERCEPT_PROXY_HTTPS_MITM_ENABLE:-true}"
INTERCEPT_CA_REGENERATE="${INTERCEPT_CA_REGENERATE:-true}"
...
INTERCEPT_PROXY_PORT_RANGE=${INTERCEPT_PROXY_PORT_RANGE}
INTERCEPT_PROXY_HTTPS_MITM_ENABLE=${INTERCEPT_PROXY_HTTPS_MITM_ENABLE}
...
if [ "${INTERCEPT_CA_REGENERATE}" = "true" ]; then
  echo "[3b/12] Regenerating intercept CA..."
  rm -rf "${APP_DIR}/backend/data/intercept-ca"
fi
...
server {
  ...
  # CA certificate is generated by backend runtime, so keep this path proxied.
  location = /downloads/apik-ca.crt {
    proxy_pass http://127.0.0.1:${BACKEND_PORT};
    proxy_http_version 1.1;
    proxy_set_header Host \$host;
    proxy_set_header X-Forwarded-Proto \$scheme;
  ...
  location = /downloads/apik-ca-info {
    proxy_pass http://127.0.0.1:${BACKEND_PORT};
    proxy_http_version 1.1;
    proxy_set_header Host \$host;
    proxy_set_header X-Forwarded-Proto \$scheme;
